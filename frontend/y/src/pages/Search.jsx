...
import { ProductElement, SearchPagination, SectionTitle } from "../components/index.js";
...
const Search = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios(
        `http://localhost:8080/product/searchproduct/${e.target.search.value}`
      );
  ...
  const handleSearchPagination = async () => {
    try {
      const response = await axios(
          `http://localhost:8080/product/searchproduct/${searchTerm}`
      );
      const data = response.data;
      setProducts(data);
    } catch (error) {
