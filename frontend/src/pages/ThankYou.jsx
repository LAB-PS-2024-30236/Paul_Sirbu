...
import { calculateTotals, clearCart } from "../features/cart/cartSlice.js";
...
const ThankYou = () => {
  ...
  const saveToOrderHistory = async () => {
    try {
      const userId = await axios(`http://localhost:8080/user/getuser/${localStorage.getItem("id")}`);
      await axios.post("http://localhost:8080/weborders/saveweborder", {
        user: userId.data,
        addressId: 2,
        address: "Str Titu Maiorescu",
        quantities: getCartItems(),
      });
      ...
      <>
        ...
        <div className="thankyou-content text-center text-accent-content px-10 max-w-7xl mx-auto">
          ...
          <h4 className="text-xl max-sm:text-lg">
            Sincerely, Pravalia lui Paul team
          </h4>
        </div>
      </>
  );
